...
/**
 * Poll the desktop app for captcha requests.
 */
async function pollForRequests() {
    if (!isPolling) return;

    try {
        const response = await fetch(`${APP_BASE}/captcha/request`, {
            method: 'GET',
        ...
        if (response.status === 200) {
            ...
            if (data.need_token) {
                ...
                console.log(`[Whisk Captcha] 📥 Got request: action=${action}, count=${count}`);
                ...
                try {
                    ...
                    if (result?.success && result.tokens?.length > 0) {
                        // Send tokens back to desktop app
                        try {
                            const postResp = await fetch(`${APP_BASE}/captcha/token`, {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ tokens: result.tokens, action }),
                                signal: AbortSignal.timeout(3000),
                            });
                            console.log(`[Whisk Captcha] ✅ Sent ${result.tokens.length} token(s), status=${postResp.status}`);
                        } catch (e) {
        ...
        chrome.runtime.sendMessage({ type: 'STATUS_UPDATE', status: 'polling' });
 ...
 */
function start() {
    console.log('[Whisk Captcha] 🔌 Starting captcha bridge (content script)...');
